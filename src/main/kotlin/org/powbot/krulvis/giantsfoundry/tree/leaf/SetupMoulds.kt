...
class SetupMoulds(script: GiantsFoundry) : Leaf<GiantsFoundry>(script, "Setup moulds") {
    ...
    private fun selectPage(mouldType: MouldType): Boolean {
        ...
        return button.click() && waitFor { MouldType.openPage() == mouldType }
    }
    ...
    private fun handleGlitch() {
        ...
        script.logger.info(
            "Glitch with selected=[${MouldType.values().map { it.selected() }}], textures=[${MouldType.values().map { it.selectedTexture() }}], indexOfSelector=$index"
        )
        resetButton()?.click()
    }
